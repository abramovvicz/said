...
public class ProtocolExtractor implements ResultSetExtractor<List<Protocol>> {


    @Override
    public List<Protocol> extractData(ResultSet resultSet) throws SQLException, DataAccessException {
        Map<Integer, Protocol> protocolMap = new HashMap<>();
        Map<Integer, Measurement> measurementMap = new HashMap();

        while (resultSet.next()) {
            var protocolId = resultSet.getInt("protocol.id");
            var protocol = protocolMap.get(Objects.isNull(protocolId) ? 0 : protocolId);
            if (protocol == null) {
                var id = resultSet.getInt("protocol.id");
                var protocolTitle = resultSet.getString("title");
                protocol = new Protocol();
                protocol.setId(id);
                protocol.setTitle(StringUtils.isEmpty(protocolTitle) ? "" : protocolTitle);
                protocol.setMeasurements(new ArrayList<>());
            }
            ...
            if (measurement == null) {
                ...
                var title = resultSet.getString("measurements.title");
                var protocolIdInMeasurement = resultSet.getInt("protocol_id");
                ...
                measurement.setProtocolId(protocolIdInMeasurement);
            ...
            protocol.getMeasurements().add(measurement);

            protocolMap.put(protocolId, protocol);
        }
        return List.copyOf(protocolMap.values());
    }
}
